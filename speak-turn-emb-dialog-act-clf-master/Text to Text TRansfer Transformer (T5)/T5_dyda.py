...
# ✅ Step 1: Move out of the cloned repo folder
# %cd /content

# ✅ Step 2: Confirm you're not in the repo
!pwd  # Should show: /content

# ✅ Step 3: Check for conflicting file
!ls speak-turn-emb-dialog-act-clf | grep datasets.py  # Just to confirm it's the issue

# ❌ Delete the wrong datasets.py file from /content
...
for dataset in datasets:
    ...
    for split in splits:
        ...
        if os.path.exists(csv_path):
            ...
            df.to_json(json_path, orient='records', lines=True)  # ✅ fixed
...
print("✅ Converted val.csv → valid.json")
...
def preprocess(example):
    input_text = "classify: " + str(example["text"])  # 👈 force to string
    label_text = str(example["act"])                  # 👈 force to string
...
def run_t5(dataset_name):
    print(f"\n🚀 Running T5 on {dataset_name.upper()}")
    ...
    trainer.train()
    print(f"\n✅ Evaluation on {dataset_name.upper()} test set:")
...
# 🔁 Settings specific for SwDA
EPOCHS = 10
BATCH_SIZE = 4
MAX_INPUT_LEN = 128
MAX_LABEL_LEN = 8
DATASETS = ['swda']  # ✅ Only SwDA
...
# Rename and convert val.csv to valid.json
df = pd.read_csv("data/dyda/val.csv")
df.to_json("data/dyda/valid.json", orient='records', lines=True)
...
# 🔁 Settings specific for dyda
EPOCHS = 10
BATCH_SIZE = 4
MAX_INPUT_LEN = 128
MAX_LABEL_LEN = 8
DATASETS = ['dyda']  # ✅ Only dyda
