...
def evaluate_multiple_sample_sizes(dataset_name, k=5, sample_sizes=[5, 10, 50, 100]):
    results = []
    for size in sample_sizes:
        print(f"\n🔁 Running {dataset_name.upper()} with {size} test samples")
        train_data, test_data = load_dataset_local(dataset_name)
        correct = 0
        for i in range(size):
            few_shot_examples = random.sample(train_data, k)
        ...
        print(f"✅ Accuracy with {size} samples: {accuracy:.2f}")
        results.append((dataset_name.upper(), size, accuracy))
    return results

import pandas as pd

all_results = []
for dataset in ["mrda", "swda", "dyda"]:
    result = evaluate_multiple_sample_sizes(dataset, k=5, sample_sizes=[5, 10, 50, 100])
    all_results.extend(result)


df = pd.DataFrame(all_results, columns=["Dataset", "Test Samples", "Accuracy"])
print("\n📊 Summary Table:")
print(df)